...
const MatchHero = ({ customImage, bgType = "image", bgColor = "#8e160b", imagePosition = 50 }: { customImage?: string | null, bgType?: string, bgColor?: string, imagePosition?: number }) => {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden flex flex-col justify-center py-14 md:py-20">
      ...
      <div className="relative container mx-auto px-4 md:px-12 flex flex-col justify-center h-full max-w-7xl">
        ...
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="flex items-stretch gap-6 md:gap-10 mb-10 md:mb-14">
          ...
          <div className="bg-[#2a2a2c] flex flex-col items-center justify-start pt-6 md:pt-10 px-6 md:px-8 border-l-[6px] border-[#8e160b] shadow-2xl min-w-[100px] md:min-w-[140px]">
            <span className="font-heading text-5xl md:text-[80px] text-white leading-none">{format(parseISO(nextMatch.date), "dd")}</span>
            <span className="font-heading text-2xl md:text-[32px] text-white/70 mt-3 md:mt-4">{format(parseISO(nextMatch.date), "MMM")}</span>
          </div>

          {/* Teams Text Block */}
          <div className="flex flex-col justify-center">
            <p className="font-heading text-sm md:text-xl text-[#8e160b] tracking-[0.2em] mb-4 md:mb-6 uppercase">
              {competition} · {nextMatch.time}
            </p>
            <div className="font-heading text-5xl md:text-[90px] lg:text-[110px] leading-[0.9] text-white drop-shadow-xl uppercase">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-wrap items-center gap-6 md:gap-10">
          ...
          <Link to="/matches" className="bg-transparent border border-white/60 hover:bg-white hover:text-black text-white font-heading text-xl md:text-2xl uppercase tracking-wider px-10 py-4 transition-all whitespace-nowrap">
            Match Centre
          </Link>

          {/* Simple Inline Countdown */}
          <div className="flex items-center gap-4 bg-black/40 backdrop-blur-sm px-6 py-3 border border-white/10">
             <div className="text-center"><span className="font-heading text-3xl md:text-4xl text-white">{String(countdown.days).padStart(2,'0')}</span><span className="block text-[10px] md:text-xs text-white/50 font-heading tracking-widest mt-1">DAYS</span></div>
             <span className="font-heading text-2xl md:text-3xl text-white/20 mb-4">:</span>
             <div className="text-center"><span className="font-heading text-3xl md:text-4xl text-white">{String(countdown.hours).padStart(2,'0')}</span><span className="block text-[10px] md:text-xs text-white/50 font-heading tracking-widest mt-1">HRS</span></div>
             <span className="font-heading text-2xl md:text-3xl text-white/20 mb-4">:</span>
             <div className="text-center"><span className="font-heading text-3xl md:text-4xl text-white">{String(countdown.mins).padStart(2,'0')}</span><span className="block text-[10px] md:text-xs text-white/50 font-heading tracking-widest mt-1">MIN</span></div>
             <span className="font-heading text-2xl md:text-3xl text-white/20 mb-4">:</span>
             <div className="text-center"><span className="font-heading text-3xl md:text-4xl text-white">{String(countdown.secs).padStart(2,'0')}</span><span className="block text-[10px] md:text-xs text-white/50 font-heading tracking-widest mt-1">SEC</span></div>
          </div>
